...
def _create_indexes(cursor):
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_codes_created_at ON auth_codes(created_at)")
...
def get_auth_code_entry(email, purpose):
    return get_db().execute(
        """
        SELECT email, purpose, code_hash, created_at, attempts
        FROM auth_codes
...
def store_auth_code(email, purpose, code_hash, created_at):
    db = get_db()
    db.execute(
        """
        INSERT INTO auth_codes (email, purpose, code_hash, created_at, attempts)
        VALUES (?, ?, ?, ?, 0)
        ON CONFLICT(email, purpose) DO UPDATE SET
            ...
            attempts = 0
        """,
        (email, purpose, code_hash, int(created_at)),
    )
    db.commit()


def increment_auth_code_attempts(email, purpose):
    db = get_db()
    db.execute(
        "UPDATE auth_codes SET attempts = attempts + 1 WHERE email = ? AND purpose = ?",
        (email, purpose),
    )
    row = db.execute(
        "SELECT attempts FROM auth_codes WHERE email = ? AND purpose = ?",
        (email, purpose),
    ).fetchone()
    db.commit()
...
def delete_auth_code(email, purpose):
    db = get_db()
    db.execute(
        "DELETE FROM auth_codes WHERE email = ? AND purpose = ?",
        (email, purpose),
    )
    db.commit()


def purge_expired_auth_codes(oldest_allowed_created_at):
    db = get_db()
    db.execute(
        "DELETE FROM auth_codes WHERE created_at < ?",
        (int(oldest_allowed_created_at),),
    )
    db.commit()
